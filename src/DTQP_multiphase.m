%--------------------------------------------------------------------------
% DTQP_multiphase.m
% construct and solve the LQDO multiphase problem
%--------------------------------------------------------------------------
%
%--------------------------------------------------------------------------
...
function [T,U,Y,P,F,p,opts] = DTQP_multiphase(SETUP,opts)
    ...
    for idx = 1:length(SETUP)
        ...
        [Hi,fi,ci,Ai,bi,Aeqi,beqi,lbi,ubi,setup(idx),p(idx),opts] = DTQP_create(SETUP(idx),opts);
    ...
    % get unique values (assumes continuity constraints are satisfied)
    [T, IT, ~] = unique(T);
